...
class MotorDriver extends Thread {
  ...
  val ai = new NeuroEvolution(n_Bit_Weight = 3, n_Bit_Bias = 3, numberOfNodes = Array(1, 8, 16, 8, 2), activationFunction = TweakedCosine,
    popSize = 16, pSelection = 0.25, pMutationPow = 8, aMutationPow = 4, parent_immutable = true,
  ...
  override def run = {
    ...
    while (true) {
      ...
      roundPerSecond = roundPerSecond * (1 - alpha) + alpha * (nowRound - lastRound) * 1000 / (nowTime - lastTime)
      var message = "\nRound: \t" + nowRound + "\t(" + Math.round(roundPerSecond) + " rounds per Second)"
